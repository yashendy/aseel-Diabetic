...
/* تحميل بيانات الأطفال + ملخص اليوم */
...
function render(){
  ...
  filtered.forEach((k,idx)=>{
    ...
    card.setAttribute('aria-label',`بيانات ${k.name||'طفل'}`);
    card.innerHTML=`
      <div class="kid-head">
      ...
      <div class="kid-actions">
        <button class="btn primary kid-open" data-id="${k.id}" aria-label="فتح لوحة الطفل ${esc(k.name||'')}">📂 فتح لوحة الطفل</button>
        <button class="btn kid-ai" data-id="${k.id}" aria-label="فتح مساعد هذا الطفل ${esc(k.name||'')}">🤖 مساعد هذا الطفل</button>
      </div>
    `;

    // الأزرار فقط هي اللي تنقل
    card.querySelector('.kid-open').onclick = () =>
      location.href = `child.html?child=${encodeURIComponent(k.id)}`;

    card.querySelector('.kid-ai').onclick = () => openAIForChild(k);

    // ❌ مهم: لا تجعل البطاقة نفسها زرًّا — لا onClick على البطاقة
...
function appendMsg(role,text){
  ...
  d.className=role==='assistant'?'msg assistant':(role==='system'?'msg sys':'msg user');
  d.textContent=text;
...
function openAIForChild(child){
  ...
  appendMsg('system','تم فتح المساعد بسياق هذا الطفل.');
}
function openAIGeneric(){
  aiState.child=null; aiState.chatSession=null;
  openAIWidget();
  aiContext.textContent='بدون سياق طفل';
  appendMsg('system','مرحبًا! اسأليني عن الجرعات، الوجبات، والمقاييس.');
}
...
async function sendAI(){
  ...
  try{
    const reply=await callGemini(buildSystemPrompt(aiState.child), text);
    waitEl.remove(); appendMsg('assistant',reply);
  }catch(e){
    waitEl.remove(); appendMsg('assistant','تعذّر الاتصال بالمساعد.');
    console.error(e);
  }
}
