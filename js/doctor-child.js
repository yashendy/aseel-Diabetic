// ملف الطفل للطبيب — عرض أسبوع قياسات + آخر 3 وجبات (Phase 0 - v2)
...
onAuthStateChanged(auth, async (user)=>{
  ...
  // تحقق مبسّط — الحماية الحقيقية في Rules
...
async function loadWeekMeasurements(pUid, cId, child, unit){
  ...
  // بما أن الحقل date بصيغة YYYY-MM-DD نقدر نفلتر بنطاق
  ...
  snap.forEach(s=>{
    ...
    // لو فيه أكثر من قراءة لنفس الخانة في اليوم نأخذ الأحدث
...
function renderWeekTable(rows, child, unit){
  ...
  rows.forEach(r=>{
    ...
    // لكل عمود (نفس ترتيب SLOT_COLUMNS)
    SLOT_COLUMNS.forEach((col, idx)=>{
      const td = document.createElement('td');
      const cell = r.cells[idx];
      if (cell && cell.val!=null){
        const cls = classify(cell.val, rangeMin, rangeMax, unit);
        td.className = `cell ${cls}`;
        td.innerHTML = `
          <span class="val">${escapeHTML(String(cell.val))}</span>
          ${cell.note ? `<span class="note">${escapeHTML(cell.note)}</span>` : ''}`;
      } else {
        td.textContent = '—';
      }
      tr.appendChild(td);
...
function formatDateAr(yyyy_mm_dd){
  // 2025-08-24 → 24-08-2025
  const [y,m,d] = (yyyy_mm_dd||'').split('-'); return `${d}-${m}-${y}`;
}

function normalizeSlot(s){
  // نوحّد الخانات لأسماء الأعمدة المعروضة
  const x = (s||'').trim();
  const map = {
...
function classify(val, min, max, unit){
  // لو مفيش نطاق محدد نعتبر كله طبيعي
  if (min==null || max==null) return 'ok';
  // val بالوحدة المعروضة بالفعل
