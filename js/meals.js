// js/meals.js — يقرأ من مكتبة الأدمن العامة + نفس سلوك الصفحة
...
function netCarbOf(item, grams, useNetMode){
  ...
  const c=n.carbs_g||0, f=n.fiber_g||0;
  const base = useNetMode ? (c - f) : c;
  const x = grams/100 * (base>0?base:0);
  return Math.max(0, x);
}
...
function renderAIHints(){
  ...
  if(NORMAL_MIN!=null && pre < NORMAL_MIN){
    aiHints.innerHTML = `<span class="chip bad">هبوط: ${pre.toFixed(1)} — عالجي الهبوط أولًا (15g سريع) ثم أعيدي القياس</span>`;
    ...
    aiHints.innerHTML = `<span class="chip warn">مرتفع: ${pre.toFixed(1)} — تصحيح ≈ <strong>${fmt(corrR)}</strong> U</span>`;
  }else{
...
/* ---------- مابّينج عنصر من مكتبة الأدمن لهيكل الوجبات ---------- */
function mapAdminItem(docData){
  // يدعم الشكلين:
  // 1) nutrPer100g + measures (جاهز)
  // 2) carbs_100g/fiber_100g/... + householdUnits (نحوّلها)
  const mapped = { ...docData };

  if(!mapped.nutrPer100g){
    mapped.nutrPer100g = {
      carbs_g:   Number(docData.carbs_100g ?? 0),
      fiber_g:   Number(docData.fiber_100g ?? 0),
      protein_g: Number(docData.protein_100g ?? 0),
      fat_g:     Number(docData.fat_100g ?? 0),
      cal_kcal:  Number(docData.calories_100g ?? 0),
    };
  }
  if(!mapped.measures && Array.isArray(docData.householdUnits)){
    mapped.measures = docData.householdUnits.map(u=>({ name:u.name, grams:Number(u.grams) }));
  }
  return mapped;
}

/* ---------- تحميل كتالوج الأصناف (من الأدمن) ---------- */
async function loadCatalog(){
  ...
  ITEMS = snap.docs.map(d => ({ id:d.id, ...mapAdminItem(d.data()) }));
  renderGrid();
}
...
async function loadDayMeals(dateStr){
  ...
  if(!rows.length){
    tbody.innerHTML = '<tr><td colspan="8">لا توجد وجبات اليوم.</td></tr>';
    sumNet.textContent='صافي الكارب: — g';
    sumBolus.textContent='مجموع الجرعات المقترَحة: — U';
    return;
  }
...
function renderGrid(){
  ...
  arr.forEach(it=>{
    ...
    const kcal = n.cal_kcal ?? calcCalories(n.carbs_g, n.protein_g, n.fat_g);
    ...
    card.innerHTML = `
      ...
      <div class="row two" style="margin-top:8px">
        ...
        <select class="input unitSel">
          ...
          ${(it.measures||[]).map(u=>`<option value="${u.grams}">${esc(u.name)} (${u.grams}g)</option>`).join('')}
        </select>
      </div>
      <div class="row two">
    ...
    // اقتراح كمية للوصول لهدف الوجبة (لو متاح)
    card.querySelector('.qSuggest').addEventListener('click', ()=>{
      ...
      const basePer100 = (useNet.checked ? ((n.carbs_g||0)-(n.fiber_g||0)) : (n.carbs_g||0));
    ...
    card.querySelector('.qAdd').addEventListener('click', async ()=>{
      ...
      try{
        ...
        await addDoc(ref, {
          ...
          nutrPer100g: n,            // <— للاستخدام في التقارير
...
onAuthStateChanged(auth, async (user)=>{
  ...
  try{
    ...
    renderAIHints();
    await loadCatalog();          // ← القراءة من admin/global/foodItems
    await loadDayMeals(dayEl.value);
  }catch(e){
    console.error(e);
